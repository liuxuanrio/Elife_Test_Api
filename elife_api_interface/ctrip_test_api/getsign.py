...
def day_ctripMd5Str(timestamp, bodyLenth):
    ...
    string = get_ctrip_vendor_id() + '3.0' + str(timestamp) + get_ctrip_secrect_key(env) + str(bodyLenth)
    # string = string.lower()
...
def ctripMd5Str(timestamp, bodyLenth,env):
    """
    请求ctrip签名
    """
    print(timestamp, bodyLenth)
    string = get_ctrip_vendor_id() + '2.0' + 'JNT' + str(timestamp) + get_ctrip_secrect_key(env) + str(bodyLenth)
    res = hashlib.md5(string.encode('utf-8')).hexdigest()
    return res

def ctripMd5(version, channel, timestamp, bodyLenth, signStr,env):
    """
    ctrip 签名验证
    """
    string = get_ctrip_vendor_id() + str(version) + str(channel) + str(timestamp) + get_ctrip_secrect_key(env) + str(bodyLenth)
    res = hashlib.md5(string.encode('utf-8')).hexdigest()
    print("需签名串：%s, 签名后：%s, 签名串：%s" % (string, res, signStr))
    return res == signStr

def ctrip_request(data,urlstr,env):
    try:
        dec = CDes(get_ctrip_secrect_key(env))
        enData = dec.encrypt(data)
        timeStamp = get_shanghai_time()
        url_info = get_ctrip_url_info()
        signStr = ctripMd5Str(timeStamp, len(enData),env)
